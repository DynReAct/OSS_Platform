...
class PerformanceEstimation(BaseModel):

    performance: float = Field(..., description="A value between 0 and 1. Zero indicates inavailability of the plant to "
                              + "process the requested order or coil, One indicates no restriction.")
    equipment: int
    order: str
    material: str | None = None
    reason: str|None = None
    estimated_validity: datetime|None = None
...
class PlantPerformanceModel:
    ...
    def bulk_performance(self, plant: int, orders: list[Order], coils: list[Material] | None = None) -> list[PerformanceEstimation]:
        results = None
