...
def calculate_bidding_price(material_params: dict, equipment_status: dict, previous_price: float | None) -> float | None:
    ...
    # JOM 2025
    if equipment_status['equipment'] not in material_params['order']['allowed_equipment']:
        return None
...
def calculate_production_cost(material_params: dict, equipment_status: dict) -> float | None:
    """
    ...
    :rtype: float
    """
    prod_cost, _ = get_transition_cost_and_status(material_params=material_params, equipment_status=equipment_status)
    return prod_cost


def get_new_equipment_status(material_params: dict, equipment_status: dict) -> dict | None:
    """
    ...
    :rtype: float
    """
    _, new_status = get_transition_cost_and_status(material_params=material_params, equipment_status=equipment_status)
    return new_status
